...
#' @param error Vector of differences betwwen predicted and observed values
...
#' @title get_v_t
#' @description Get a vector of n_gas evenly spaced times up to x_max
#' @param x_max Maximum time for chamber enclosure
#' @param n_gas Number of gas samples taken over the measurement
#' @return A vector of evenly spaced times
#' @details DETAILS
#' @examples
#' \dontrun{
#' if(interactive()){
...
#' @title get_sigma_t
#' @description Get the standard deviation of the time series
#' @param v_t A vector of times
...
#' This is an attempt to compare sampling from normal versus lognormal
#' distributions. Not sure we can make these comparable meaningfully.
...
#' @param rel_error PARAM_DESCRIPTION, Default: FALSE
#' @param plot_graph PARAM_DESCRIPTION, Default: FALSE
#' @param plot_histogram PARAM_DESCRIPTION, Default: FALSE
#' @param get_v_omega PARAM_DESCRIPTION, Default: FALSE
#' @param trt_diff_obs PARAM_DESCRIPTION, Default: 0.005
#' @return OUTPUT_DESCRIPTION
#' @details DETAILS
#' @examples
#' \dontrun{
...
get_ci_omega <- function(
  ...
  df <- data.frame(
    ...
    F_cum_ci = F_cum_rmsd * 1.96,
    omega_ci = omega_rmsd * 1.96,
    omega_ci_lo = quantile(v_omega_obs, probs = 0.025),
    omega_ci_hi = quantile(v_omega_obs, probs = 0.975),
    # normalise wrt to true value, and express as a percentage of it
    omega_ci_norm_pc = omega_rmsd * 1.96 / omega * 100,
    F_cum_bias_norm_pc = mean(v_F_cum_error) / F_cum * 100,
    omega_bias_norm_pc = mean(v_omega_error) / omega * 100,
    ci_flux = ci_flux,
    df_params)

  if (plot_histogram) {
    cols <- c("Treatment diff" = "black", "CI 95%" = "red")
    fills <- c("True mean" = "black", "Simulations" = "pink")
    p <- ggplot(df, aes(omega_true))
    p <- p + scale_colour_manual(name = "", values = cols)
    p <- p + scale_fill_manual(name = "", values = fills)
    p <- p + geom_histogram(data = data.frame(omega = v_omega_obs),
      aes(x = omega, fill = "Simulations"))
    p <- p + geom_vline(aes(xintercept = omega_true, colour = "Treatment diff"))
    p <- p + geom_vline(aes(xintercept = omega_true + trt_diff_obs,
      colour = "Treatment diff"))
    p <- p + geom_vline(aes(xintercept = omega_ci_lo, colour = "CI 95%"),
      linetype = "dotted")
    p <- p + geom_vline(aes(xintercept = omega_ci_hi, colour = "CI 95%"),
      linetype = "dotted")
    p <- p + xlab("Emission factor")
    print(p)
  }
...
#' DNDC Model output data
#'
#' Output data from some example runs of the DNDC model
#'
#' @format ## `df_dndc`
#' A data frame with 88 rows and 5 columns:
#' \describe{
#'   \item{site}{Site name}
#'   \item{days_since_fert, secs_since_fert}{Time since fertiliser applied in days or seconds}
#'   \item{Fn2o_kgN_ha_d, Fn2o_nmol_m2_s}{Predicted flux of N2O in given units}
#' }
#' @source <https://www.dndc.sr.unh.edu/>
"df_dndc"
